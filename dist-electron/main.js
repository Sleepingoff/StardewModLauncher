...
const CONFIG_PATH = path.join(__dirname$1, "config.json");
const MODS_DIR = path.join(__dirname$1, "Mods");
const dir = {
...
function buildModMapRecursive(dir2, map = {}) {
  if (!fs.existsSync(dir2)) return map;
...
function loadConfig(presetName) {
  if (!fs.existsSync(dir.CONFIG_PATH)) return {};
  const raw = JSON.parse(fs.readFileSync(dir.CONFIG_PATH, "utf-8"));
...
function folderTreeToConfig(folderTree, baseDir, presetName) {
  ...
  function traverse(tree, currentPath) {
    for (const key2 of Object.keys(tree)) {
      const fullPath = path.join(currentPath, key2);
      const manifestPath = path.join(fullPath, "manifest.json");
...
function configToFolderTree(config) {
  ...
  for (const [uniqueId, { enabled }] of Object.entries(config)) {
    ...
    if (!modPath) continue;
    const relativePath = path.relative(dir.MODS_DIR, modPath);
...
function loadAllPresets() {
  if (!fs.existsSync(dir.CONFIG_PATH)) return {};
  return JSON.parse(fs.readFileSync(dir.CONFIG_PATH, "utf-8"));
}
...
function getModsFromDisk() {
  ...
  return fs.readdirSync(modsPath).filter((dir2) => {
    const fullPath = path.join(modsPath, dir2);
...
function createWindow() {
  win = new BrowserWindow({
    ...
    height: 800,
    webPreferences: {
      preload: path.join(dir.__dirname, "preload.mjs"),
...
function scanModsTreeByManifest(dir2) {
  ...
  for (const entry of entries) {
    const fullPath = path.join(dir2, entry);
...
ipcMain.handle("get-translations", (event, locale) => {
  const filePath = path.join(dir.__dirname, `${locale}.json`);
