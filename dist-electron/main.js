...
function saveConfig(folderTree, presetName) {
  const newPreset = folderTreeToConfig(folderTree, dir.MODS_DIR);
...
function folderTreeToConfig(folderTree, baseDir) {
  const flatMap = {};
  function traverse(tree, currentPath) {
    for (const [key2, value] of Object.entries(tree)) {
