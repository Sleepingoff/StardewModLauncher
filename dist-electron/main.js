...
function buildModMapRecursive(dir, map = {}) {
  if (!fs$j.existsSync(dir)) return map;
  const entries = fs$j.readdirSync(dir, { withFileTypes: true });
  for (const entry of entries) {
    const fullPath = path$c.join(dir, entry.name);
...
function loadConfig(configPath) {
  if (!fs$j.existsSync(configPath)) return {};
  return JSON.parse(fs$j.readFileSync(configPath, "utf-8"));
}
function saveConfig(configPath, modsDir, modConfig) {
  var _a;
  ...
  for (const preset of presets) {
    ...
    for (const uniqueId of Object.keys(modMap)) {
      config[preset][uniqueId] = ((_a = modConfig[preset]) == null ? void 0 : _a[uniqueId]) ?? false;
    }
  }
  fs$j.writeFileSync(configPath, JSON.stringify(config, null, 2), "utf-8");
}
...
function scanModsTreeByManifest(dir) {
  if (!fs.existsSync(dir)) return {};
  const tree = {};
  const entries = fs.readdirSync(dir);
  for (const entry of entries) {
    const fullPath = path$c.join(dir, entry);
...
ipcMain.handle("reset-mods", async (_event, { smapiPath, modStates }) => {
  ...
  for (const uniqueId of Object.keys(modStates)) {
    const modPath = path$c.join(modsUserPath, uniqueId);
...
const localesDir = path$c.join(__dirname, "locales");
let currentLang = "ko";
function loadLocale(lang) {
  const filePath = path$c.join(localesDir, `${lang}.json`);
