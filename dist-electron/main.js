...
import p from "path";
import { app as Ju, ipcMain as Y, shell as Pt, BrowserWindow as kt } from "electron";
...
var au = vt, xt = process.cwd, Nu = null, It = process.env.GRACEFUL_FS_PLATFORM || process.platform;
process.cwd = function() {
  return Nu || (Nu = xt.call(process)), Nu;
};
try {
  process.cwd();
} catch {
  ...
  function n(c) {
    return c && function(l, A, C) {
      return c.call(u, l, A, function(y) {
        m(y) && (y = null), C && C.apply(this, arguments);
      });
    };
  }
  function r(c) {
  ...
  function i(c) {
    return c && function(l, A, C, y) {
      return c.call(u, l, A, C, function(f) {
        m(f) && (f = null), y && y.apply(this, arguments);
      });
    };
  }
  function o(c) {
...
var N = R, Wt = Nt, qt = Tt, Ut = Mt, Ou = bt, z, ju;
typeof Symbol == "function" && typeof Symbol.for == "function" ? (z = Symbol.for("graceful-fs.queue"), ju = Symbol.for("graceful-fs.previous")) : (z = "___graceful-fs.queue", ju = "___graceful-fs.previous");
function Vt() {
}
function xe(u, e) {
...
var Eu = Vt;
Ou.debuglog ? Eu = Ou.debuglog("gfs4") : /\bgfs4\b/i.test(process.env.NODE_DEBUG || "") && (Eu = function() {
  var u = Ou.format.apply(Ou, arguments);
  u = "GFS4: " + u.split(/\n/).join(`
...
if (!N[z]) {
  ...
  xe(N, Ht), N.close = function(u) {
    ...
    return Object.defineProperty(e, ju, {
      value: u
    }), e;
...
Zu[z] || xe(Zu, N[z]);
var pu = ie(Ut(N));
process.env.TEST_GRACEFUL_FS_GLOBAL_PATCH && !N.__patched && (pu = ie(N), N.__patched = !0);
function ie(u) {
  Wt(u), u.gracefulify = ie, u.createReadStream = M, u.createWriteStream = ru;
  var e = u.readFile;
  u.readFile = t;
  function t(d, b, g) {
  ...
  function w() {
    var d = this;
    Hu(d.path, d.flags, d.mode, function(b, g) {
...
function yu(u) {
  Eu("ENQUEUE", u[0].name, u[1]), N[z].push(u), oe();
}
var _u;
function me() {
...
function oe() {
  if (clearTimeout(_u), _u = void 0, N[z].length !== 0) {
    ...
    if (r === void 0)
      ...
      var a = Date.now() - i, F = Math.max(i - r, 1), m = Math.min(F * 1.2, 100);
      a >= m ? (Eu("RETRY", e.name, t), e.apply(null, t.concat([r]))) : N[z].push(u);
    }
    _u === void 0 && (_u = setTimeout(oe, 0));
  }
}
(function(u) {
  ...
  }, u.read = function(r, i, o, a, F, m) {
    return typeof m == "function" ? t.read(r, i, o, a, F, m) : new Promise((c, l) => {
...
De.makeDir = async (u, e) => (je(u), Ne.mkdir(u, {
  mode: Te(e),
  recursive: !0
}));
De.makeDirSync = (u, e) => (je(u), Ne.mkdirSync(u, {
  mode: Te(e),
  recursive: !0
}));
const zt = J.fromPromise, { makeDir: Yt, makeDirSync: Gu } = De, zu = zt(Yt);
var nu = {
  mkdirs: zu,
  mkdirsSync: Gu,
  // alias
...
async function nn(u, e, t, n) {
  ...
  if (r.isDirectory() && ce(u, e))
    throw new Error(Wu(u, e, t));
...
function Ve(u, e, t, n) {
  return X.copyFileSync(e, t), n.preserveTimestamps && pn(u.mode, e, t), ae(t, u.mode);
}
function pn(u, e, t) {
...
function gn(u, e, t, n) {
  return X.mkdirSync(t), He(e, t, n), ae(t, u);
}
function He(u, e, t) {
...
const Nn = J.fromPromise, ze = uu, Ye = p, Ke = nu, Qe = qu, pe = Nn(async function(e) {
  let t;
  try {
...
var fe = { stringify: Fr, stripBom: Cr };
let hu;
try {
  hu = pu;
} catch {
...
async function mr(u, e, t = {}) {
  ...
  await Uu.fromCallback(n.writeFile)(u, r, t);
}
const dr = Uu.fromPromise(mr);
function hr(u, e, t = {}) {
  const n = t.fs || hu, r = ot(e, t);
...
var Br = {
  // jsonfile exports
  readJson: xu.readFile,
  readJsonSync: xu.readFileSync,
  writeJson: xu.writeFile,
  writeJsonSync: xu.writeFileSync
};
const Sr = J.fromPromise, ue = uu, ct = p, at = nu, wr = Au.pathExists;
async function vr(u, e, t = "utf-8") {
  const n = ct.dirname(u);
  return await wr(n) || await at.mkdirs(n), ue.writeFile(u, e, t);
}
function gr(u, ...e) {
  const t = ct.dirname(u);
  ue.existsSync(t) || at.mkdirsSync(t), ue.writeFileSync(u, ...e);
}
var le = {
  outputFile: Sr(vr),
  outputFileSync: gr
};
const { stringify: br } = fe, { outputFile: $r } = le;
async function Pr(u, e, t = {}) {
  const n = br(e, t);
  await $r(u, n, t);
}
var kr = Pr;
const { stringify: Or } = fe, { outputFileSync: _r } = le;
function xr(u, e, t) {
  const n = Or(e, t);
  _r(u, n, t);
}
...
var jr = Z;
const Tr = uu, Pe = p, { copy: Lr } = se, { remove: st } = qu, { mkdirp: Mr } = nu, { pathExists: Rr } = Au, ke = Bu;
...
var Ur = Jr;
const ft = pu, ee = p, Vr = se.copySync, lt = qu.removeSync, Hr = nu.mkdirpSync, Oe = Bu;
function Gr(u, e, t) {
  t = t || {};
  const n = t.overwrite || t.clobber || !1, { srcStat: r, isChangingCase: i = !1 } = Oe.checkPathsSync(u, e, "move", t);
  return Oe.checkParentPathsSync(u, r, e, "move"), zr(e) || Hr(ee.dirname(e)), Yr(u, e, n, i);
}
function zr(u) {
  const e = ee.dirname(u);
  return ee.parse(e).root === e;
}
function Yr(u, e, t, n) {
  if (n) return Yu(u, e, t);
  if (t)
    return lt(e), Yu(u, e, t);
...
const T = /* @__PURE__ */ _t(ui), Ft = Ot(import.meta.url), ei = p.dirname(Ft), Fe = Ju.getPath("userData"), ti = p.join(Fe, "config.json"), ni = p.join(Fe, "info.json"), ri = p.join(Fe, "Mods"), k = {
  __filename: Ft,
  __dirname: ei,
  CONFIG_PATH: ti,
  INFO_PATH: ni,
...
let te, Q, Du, Tu, lu, eu, q, Ce, vu;
var ci = function(e, t) {
  te = String(e), Q = "start", Du = [], Tu = 0, lu = 1, eu = 0, q = void 0, Ce = void 0, vu = void 0;
  do
    q = ai(), li[Q]();
  while (q.type !== "eof");
  return typeof t == "function" ? ne({ "": vu }, "", t) : vu;
};
function ne(u, e, t) {
  const n = u[e];
...
function cu() {
  if (te[Tu])
    return String.fromCodePoint(te.codePointAt(Tu));
}
function D() {
  const u = cu();
  return u === `
` ? (lu++, eu = 0) : u ? eu += u.length : eu++, u && (Tu += u.length), u;
}
const Ct = {
  default() {
    switch (S) {
...
`:
  ...
  value() {
    switch (S) {
      case "{":
      case "[":
        return O("punctuator", D());
      case "n":
        return D(), Fu("ull"), O("null", null);
      case "t":
        return D(), Fu("rue"), O("boolean", !0);
      case "f":
        return D(), Fu("alse"), O("boolean", !1);
      case "-":
      case "+":
        D() === "-" && (iu = -1), B = "sign";
        return;
      ...
      case "9":
        ...
        return;
      case "I":
        return D(), Fu("nfinity"), O("numeric", 1 / 0);
      case "N":
        return D(), Fu("aN"), O("numeric", NaN);
      case '"':
      case "'":
        Su = D() === '"', h = "", B = "string";
        return;
  ...
  sign() {
    switch (S) {
      ...
      case "9":
        ...
        return;
      case "I":
        return D(), Fu("nfinity"), O("numeric", iu * (1 / 0));
      case "N":
        return D(), Fu("aN"), O("numeric", NaN);
    }
    throw _(D());
...
const li = {
  ...
  beforePropertyName() {
    switch (q.type) {
      case "identifier":
      case "string":
        Ce = q.value, Q = "afterPropertyName";
        return;
      case "punctuator":
        Iu();
        return;
...
function Qu() {
  ...
  if (vu === void 0)
    ...
    Array.isArray(e) ? e.push(u) : Object.defineProperty(e, Ce, {
      value: u,
      writable: !0,
      enumerable: !0,
      configurable: !0
...
function _(u) {
  return Lu(u === void 0 ? `JSON5: invalid end of input at ${lu}:${eu}` : `JSON5: invalid character '${Et(u)}' at ${lu}:${eu}`);
}
function Cu() {
  return Lu(`JSON5: invalid end of input at ${lu}:${eu}`);
}
function _e() {
  return eu -= 5, Lu(`JSON5: invalid identifier character at ${lu}:${eu}`);
}
function Fi(u) {
...
var Ci = function(e, t, n) {
  ...
  function l(f) {
    ...
    for (let $ = 0; $ < f.length; $++) {
      ...
      if (M < " ") {
        let ru = M.charCodeAt(0).toString(16);
        w += "\\x" + ("00" + ru).substring(ru.length);
        continue;
      }
      w += M;
    }
    const v = m || Object.keys(E).reduce(($, M) => E[$] < E[M] ? $ : M);
...
function Mu(u, e = {}) {
  if (!R.existsSync(u)) return e;
  const t = R.readdirSync(u, { withFileTypes: !0 });
  for (const n of t) {
    ...
    const i = p.join(r, "manifest.json"), o = Ru(i);
    o ? e[o.UniqueID] = {
      uniqueId: o.UniqueID,
      name: o.Name ?? n.name,
      path: r,
...
function Ee(u) {
  R.existsSync(k.CONFIG_PATH) || R.writeFileSync(k.CONFIG_PATH, "{}");
  const e = JSON.parse(R.readFileSync(k.CONFIG_PATH, "utf-8"));
  if (!u) return e;
  let t = e[u];
  if (!t)
    return Mu(k.MODS_DIR);
  const n = Mu(k.MODS_DIR);
  for (const [r, i] of Object.entries(n))
    t[r] && (i.enabled = t[r].enabled);
  return n;
}
function mi(u, e) {
  const t = di(u, k.MODS_DIR), r = {
    ...Vu(),
    [e]: t
  };
  R.writeFileSync(
...
function Ru(u) {
  if (!R.existsSync(u)) return null;
  try {
    const e = R.readFileSync(u, "utf-8"), t = Ai.parse(e);
...
function hi(u) {
  ...
  for (const [t, { name: n, enabled: r }] of Object.entries(u)) {
    const i = pi(k.MODS_DIR, t);
    if (!i) continue;
    const a = p.relative(k.MODS_DIR, i).split(p.sep);
    let F = e;
    a.forEach((m, c) => {
      F[m] || (F[m] = {}), c === a.length - 1 && (F[m].uniqueId = t, F[m].name = n, F[m].enabled = r), F = F[m];
    });
  }
  return e;
}
function pi(u, e) {
  function t(n) {
    if (!R.existsSync(n)) return null;
    const r = R.readdirSync(n, { withFileTypes: !0 });
    for (const i of r) {
...
function Vu() {
  return R.existsSync(k.CONFIG_PATH) ? JSON.parse(R.readFileSync(k.CONFIG_PATH, "utf-8")) : {};
}
function At() {
  const u = Vu();
...
function wi() {
  const u = k.MODS_DIR;
  return R.existsSync(u) ? R.readdirSync(u).filter((e) => {
    const t = p.join(u, e);
    return R.statSync(t).isDirectory();
  }) : [];
}
function ht() {
  if (!T.existsSync(k.INFO_PATH))
    return {};
  try {
    const u = T.readFileSync(k.INFO_PATH, "utf-8");
    return JSON.parse(u);
  } catch (u) {
...
function gi() {
  Xu = new kt({
    ...
    height: 800,
    icon: p.join(
      Ju.isPackaged ? process.resourcesPath : p.join(k.__dirname, "public"),
      "Stardrop.png"
    ),
    // 아이콘 경로
    webPreferences: {
      preload: p.join(k.__dirname, "preload.mjs"),
      contextIsolation: !0,
      nodeIntegration: !1
    }
  }), process.env.VITE_DEV_SERVER_URL ? Xu.loadURL(process.env.VITE_DEV_SERVER_URL) : Xu.loadFile(p.join(k.__dirname, "../dist/index.html"));
}
Ju.whenReady().then(gi);
function pt(u) {
...
Y.handle("get-mod-list-tree", () => Bt(k.MODS_DIR));
Y.handle(
  "apply-mods",
  async (u, e, t) => {
    if (!e) throw new Error("smapiPath is not provided");
    const n = pt(e), r = Mu(k.MODS_DIR);
    for (const [i, o] of Object.entries(t)) {
      const a = o.uniqueId ?? i, F = r[a];
      if (!F || !o.enabled)
        continue;
      const m = F.path, c = p.join(n, p.basename(m));
      if (o.enabled)
        try {
          await T.copy(m, c, { overwrite: !0 });
        } catch (l) {
...
Y.handle("open-mods-folder", async () => {
  T.existsSync(k.MODS_DIR) || T.mkdirSync(k.MODS_DIR), Pt.openPath(k.MODS_DIR);
});
Y.handle("get-presets", () => Vu() || {});
      ...
      if (T.existsSync(o))
        try {
          ...
          if (!T.existsSync(i))
            ...
            T.existsSync(m) && Ru(m).UniqueID === F && T.existsSync(l) && await T.copyFile(l, c);
          }
        } catch (a) {
...
Y.handle("get-locale", () => Ju.getLocale().startsWith("ko") ? "ko" : "en");
Y.handle("get-translations", (u, e) => {
  const t = p.join(k.__dirname, "locales", `${e}.json`);
