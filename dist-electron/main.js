...
makeDir$1.makeDir = async (dir2, options) => {
  checkPath2(dir2);
  return fs$g.mkdir(dir2, {
    mode: getMode(options),
    recursive: true
  });
};
makeDir$1.makeDirSync = (dir2, options) => {
  checkPath2(dir2);
  return fs$g.mkdirSync(dir2, {
    mode: getMode(options),
    recursive: true
  });
};
...
function copyDir(src, dest, opts) {
  const dir2 = fs$b.opendirSync(src);
  try {
    let dirent;
...
const emptyDir = u$8(async function emptyDir2(dir2) {
  let items;
  try {
    items = await fs$9.readdir(dir2);
  } catch {
    return mkdir$3.mkdirs(dir2);
  }
  return Promise.all(items.map((item) => remove$1.remove(path$7.join(dir2, item))));
});
function emptyDirSync(dir2) {
  let items;
  try {
    items = fs$9.readdirSync(dir2);
  } catch {
    return mkdir$3.mkdirsSync(dir2);
  }
  items.forEach((item) => {
    item = path$7.join(dir2, item);
...
async function createFile$1(file2) {
  ...
  const dir2 = path$6.dirname(file2);
  let dirStats = null;
  try {
    dirStats = await fs$8.stat(dir2);
  } catch (err) {
    if (err.code === "ENOENT") {
      await mkdir$2.mkdirs(dir2);
...
function createFileSync$1(file2) {
  ...
  const dir2 = path$6.dirname(file2);
  try {
    if (!fs$8.statSync(dir2).isDirectory()) {
      fs$8.readdirSync(dir2);
    }
  } catch (err) {
    if (err && err.code === "ENOENT") mkdir$2.mkdirsSync(dir2);
    else throw err;
  }
...
async function createLink$1(srcpath, dstpath) {
  ...
  const dir2 = path$5.dirname(dstpath);
  const dirExists = await pathExists$4(dir2);
  if (!dirExists) {
    await mkdir$1.mkdirs(dir2);
  }
...
function createLinkSync$1(srcpath, dstpath) {
  ...
  const dir2 = path$5.dirname(dstpath);
  const dirExists = fs$7.existsSync(dir2);
  if (dirExists) return fs$7.linkSync(srcpath, dstpath);
  mkdir$1.mkdirsSync(dir2);
...
async function createSymlink$1(srcpath, dstpath, type) {
  ...
  const dir2 = path$3.dirname(dstpath);
  if (!await pathExists$2(dir2)) {
    await mkdirs(dir2);
  }
...
function createSymlinkSync$1(srcpath, dstpath, type) {
  ...
  const dir2 = path$3.dirname(dstpath);
  const exists = fs$4.existsSync(dir2);
...
async function outputFile$1(file2, data, encoding = "utf-8") {
  const dir2 = path$2.dirname(file2);
  if (!await pathExists$1(dir2)) {
...
var lib = JSON5;
const REQUIRED_MODS$1 = ["ConsoleCommands", "SaveBackup"];
function buildModMapRecursive(dir2, map = {}) {
  if (!fs$j.existsSync(dir2)) return map;
  const entries = fs$j.readdirSync(dir2, { withFileTypes: true });
  for (const entry of entries) {
    const fullPath = path$c.join(dir2, entry.name);
    if (!entry.isDirectory()) continue;
    if (REQUIRED_MODS$1.includes(entry.name)) {
      map[entry.name] = {
        uniqueId: entry.name,
    ...
    const manifestPath = path$c.join(fullPath, "manifest.json");
...
function loadConfig(presetName) {
  ...
  if (!presetConfig) {
    ...
    presetConfig = {};
    for (const [key2, value] of Object.entries(modMap)) {
      presetConfig[key2] = {
        name: value.name,
...
function saveConfig(folderTree, presetName) {
...
function folderTreeToConfig(folderTree, baseDir, presetName) {
  const presetConfig = {};
  function traverse(tree, currentPath) {
    for (const key2 of Object.keys(tree)) {
      const fullPath = path$c.join(currentPath, key2);
      const manifestPath = path$c.join(fullPath, "manifest.json");
      if (fs$j.existsSync(manifestPath)) {
        try {
          const manifest = safeParseManifest(manifestPath);
          if (manifest == null ? void 0 : manifest.UniqueID) {
            presetConfig[manifest.UniqueID] = {
              name: key2,
              enabled: tree[key2].enabled
              // 트리 안에 값이 있으면 반영
            };
          }
        } catch (err) {
      ...
      if (typeof tree[key2] === "object" && Object.keys(tree[key2]).length > 0) {
        traverse(tree[key2], fullPath);
      }
    }
  }
...
function safeParseManifest(manifestPath) {
  if (!fs$j.existsSync(manifestPath)) return null;
  try {
    const raw = fs$j.readFileSync(manifestPath, "utf-8");
...
function findModPathByUniqueId(modsDir, uniqueId) {
  function search(dir2) {
    if (!fs$j.existsSync(dir2)) return null;
    const entries = fs$j.readdirSync(dir2, { withFileTypes: true });
    for (const entry of entries) {
      const fullPath = path$c.join(dir2, entry.name);
...
function loadAllPresets() {
  ...
  return JSON.parse(fs$j.readFileSync(dir.CONFIG_PATH, "utf-8"));
}
function listPresets() {
  const all = loadAllPresets();
  return Object.keys(all);
}
function createPreset(presetName, mods) {
...
function getModsFromDisk() {
  const modsPath = dir.MODS_DIR;
  if (!fs$j.existsSync(modsPath)) return [];
  return fs$j.readdirSync(modsPath).filter((dir2) => {
    const fullPath = path$c.join(modsPath, dir2);
    return fs$j.statSync(fullPath).isDirectory();
  });
}
function readInfo() {
  if (!fs.existsSync(dir.INFO_PATH)) {
    return {};
  }
  try {
    const raw = fs.readFileSync(dir.INFO_PATH, "utf-8");
...
function scanModsTreeByManifest(dir2) {
  ...
  const tree = {};
  const entries = fs.readdirSync(dir2);
  for (const entry of entries) {
    const fullPath = path$c.join(dir2, entry);
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (fs.existsSync(manifestPath)) {
        ...
        const folderName = path$c.basename(gameEntryPath);
